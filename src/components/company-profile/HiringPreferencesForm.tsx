...
import { X, Briefcase, MapPin, Clock, DollarSign, Loader2, Save, Search, Plus } from "lucide-react";
...
const timeframes = [
  ...
  "Within 3 months", "Flexible / No rush",
];
...
export function HiringPreferencesForm() {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (suggestionsRef.current && !suggestionsRef.current.contains(e.target as Node) &&
          inputRef.current && !inputRef.current.contains(e.target as Node)) setShowSuggestions(false);
    };
  ...
  const handleSave = () => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      toast({ title: "Preferences Updated", description: "Your hiring preferences have been saved successfully." });
    }, 800);
  };
  ...
  return (
    <Card className="border-border/50 shadow-lg">
      <CardHeader>
        <CardTitle className="text-xl font-bold">Hiring Preferences</CardTitle>
        <CardDescription>Configure your hiring preferences to get personalized candidate recommendations.</CardDescription>
      </CardHeader>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {roles.map(role => (
                <Badge key={role} variant="secondary" className="gap-1 pr-1.5">
                  {role}
                  <button onClick={() => setRoles(prev => prev.filter(r => r !== role))} className="hover:text-destructive"><X className="h-3 w-3" /></button>
                </Badge>
              ))}
            </div>
          )}
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {locations.map(loc => (
                <Badge key={loc} variant="secondary" className="gap-1 pr-1.5">
                  {loc}
                  <button onClick={() => setLocations(prev => prev.filter(l => l !== loc))} className="hover:text-destructive"><X className="h-3 w-3" /></button>
                </Badge>
              ))}
            </div>
          )}
